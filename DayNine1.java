    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < points.size(); i++) {
            for (int j = 0; j < points.size(); j++) {
                ...
                //Part 2: define a rectangle that's actually very slightly smaller than the target
                //due to how contains() works with edges
                int x1 = Math.min(points.get(i).x, points.get(j).x);
                int y1 = Math.min(points.get(i).y, points.get(j).y);
                int x2 = Math.max(points.get(i).x, points.get(j).x);
                int y2 = Math.max(points.get(i).y, points.get(j).y);
                Rectangle2D.Double rect = new Rectangle2D.Double(x1+0.1, y1 + 0.1, x2 - x1 - 0.2, y2 - y1 - 0.2);
